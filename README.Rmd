...
```{r child="vignettes/partials/methods.Rmd"}
```
...
The separate models for the `work` and `full` dichotomies can be extracted via `models()`:
```{r}
...
A variety of other plots can be produced using base graphics (`matplot()`) and `ggplot2`, as described in the vignette,
