plot.nestedLogit <- function(x, x.var, others, n.x.values=100,
  ...
  if (!missing(main)) {
    ...
    if (length(values) != 0){
      main <- paste(paste(names(values[!x.var == names(values)]), "=", 
                          as.character(unlist(values[!x.var == names(values)]))),
                    collapse=", ")
      title(main, cex.main=cex.main, font.main=font.main)
    }
  }
