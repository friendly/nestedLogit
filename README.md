...
(fit by: e.g., `nnet::multinom()`) which compares response categories to
a *reference level*.
...
response with $m$ levels. It has the full suited of methods associated
with `glm()` and other model fitting methods.
...
- A more general discussion of nested dichotomies logistic regression
  ...
  `vignette("nestedLogit", package="nestedLogit")` and in the [pkgdown
...
- A variety of other plots can be produced using `ggplot()`, as
  described in the vignette, `vignette("plotting-ggplot")` and in the
  [pkgdown
