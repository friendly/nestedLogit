---
title: "Nested dichotomies logistic regression models (revised)"
...
output: 
  ...
  fig_caption: yes
bibliography: ["references-2.bib", "packages.bib"]
csl: apa.csl
vignette: >
  %\VignetteIndexEntry{Nested dichotomies logistic regression models (revised)}
...
by their highest completed level of education, taking on the values (a) less than highschool, (b) highschool graduate, (c) some post-secondary, or (d) post-secondary degree.

* Women's labor force participation is classified as (a) not working outside the home, (b) working parttime, or (c) working fulltime.
...
to the treatment of factors in the predictor variables.
...
the dichotomies),
so test statistics, such as likelihood ratio $G^2$ tests and Wald chi-square tests for
individual coefficients can be summed to give overall tests for the full polytomy.
In this way, the $m-1$ dichotomies are analogous to $m-1$
orthogonal contrasts for an $m$-level factor in an ANOVA design.

When applicable, therefore, nested logit models give the best of two worlds: 

* *dichotomous sub-models*: We can think about, analyze, and plot the separate model for each contrast among the response categories.

* *an overall model*: We can summarize, analyze, and plot the overall model for all response categories.
...
diagnosis can be interpreted in terms of the probabilities of being classed
into each of the response categories. Because of independence, analysis-of-deviance and coefficient tests for the 
full multi-category response are based on sums over the three dichotomies.
...
For a principal example, we consider the data set `Womenlf` from the `carData` package.
these data give the responses of 263 young married women, 21--30 years old, from a 1977 survey carried out by
...
a dichotomy (`work`) between women who are not working outside the home vs. those who are working
...
The two binary variables can be created by recoding `partic` 
as follows.  
```{r recode}
...
```
The relationship of `partic` to these dichotomies is as follows:
...
We can then fit the separate models for the two nested dichotomies directly:
...
uses $m-1$ calls to `dichotomy()` to create an object of class `dichotomies`
...
comparisons
```
It is mandatory to name the dichotomies, and we can optionally name the elements of each dichotomy, an option that is particularly useful for a compound category, such as `working=c("parttime", "fulltime")` in the example.
...
dichotomies. The `models` component of the object contains essentially the same `"glm"` model objects as we constructed directly as `mod.work` and `mod.full` above.

```{r names-etc}
names(wlf.nested)
names(wlf.nested$models) # equivalent: names(models(wlf.models))
lapply(models(wlf.nested), I)
```


### Methods for `"nestedLogit"` objects

```{r child="partials/methods-2.Rmd"}
```
...
The coefficients $\widehat{\beta}_{j1}$ gives the estimated change in the
...
among those who are working outside the home.
...
exp(coef(wlf.nested))
```
Thus, the odds of both working and working fulltime decrease with husband's income.
Having young children also decreases the odds of both.

**Anova**: A method for the `Anova()` function from the **car** package [@car2019]
computes Type II or III likelihood-ratio or Wald tests for each term in the model. Note that the likelihood-ratio or Wald $\chi^2$ and degrees of freedom for the `Combined Responses` is the sum of their values for the separate dichotomies; for the example:
```{r Anova}
Anova(wlf.nested)
```

**linearHypothesis**: The `linearHypothesis()` function in the **car** package provides a very general method for
computing Wald tests of specific hypotheses about individual coefficients in a model or their linear combinations.
The `linearHypothesis` function is extended by a  method for `"nestedLogit"` models.
...
`linearHypothesis()` reports these tests for each of the submodels for the dichotomies `work` and `full`, as well as for the combined model:
...
These functions make it easy to construct custom tables. For example, to extract the likelihood-ratio difference-in-`deviance` ($G^2$)  tests and compute ($G^2 / df$), you could do:
```{r}
...
<!-- Michael: Does G^2/df have any real use here, other than to compare to expectation under H_0? That is, is there any added information beyond what's already in the p-value? -->


**update**: The `update()` function makes it easy to create a new model from an old one, by adding or subtracting terms from the
model formula, specifying a new formula, or changing the observations used or contrasts for factors.

For example, you might ask, "_Does it make sense to include `region` of Canada in the model?_"  This question can be answered
...
We can see that neither `region` nor the interaction make a difference to the fit 
...
To produce multi-panel plots, it is necessary to call the `plot()` function repeatedly for
...
## More on constructing plots of `"nestedLogit"` objects

To explain plotting nested-dichotomies models works, and for better control of the details,
it is useful to describe how graphs can be constructed directly directly.
...
to `geom_line()` that adds a text label to a curve.
