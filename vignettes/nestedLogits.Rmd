...
However, by the construction of **nested** dichotomies, the submodels are 
...
provides some methods for giving compact and tidy summaries of fitted models. The `glance` method for a `"nested"` model gives a one-line summary of the statistics
...
**update**: Does it make sense to include `region` of Canada in the model?  
...
## Plotting
The `nestedLogit` package has a basic `plot()` method for `"nested"` models, but to explain how this
works, and for better control
