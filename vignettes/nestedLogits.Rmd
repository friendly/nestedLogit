...
However, often the response variable is multi-category ("_polytomous_"),
taking on $m > 2$ discrete values.
For example, 

* Respondents in the U.S. General Social Survey are classified
by their level of education, taking values `<highschool`, `highschool`, `college` and
`graduate`.

* Women's labor force participation might be classified as (a) not working outside the home; (b) working parttime; or (c) working fulltime.

There are several different ways to model the response probabilities.
Let \(\pi_{ij} \equiv \pi_j \,  ( \vec{x}_i )\) 
be the probability of response $j$ for case
or group $i$, given the predictors $\vec{x}_i$.
Because \(\sum_j \,  \pi_{ij} = 1\), only \(m - 1\) of
these probabilities are independent.  The essential idea is to
...
### Multinomial model
One natural generalization of the the standard logistic regression _logit_ model is the _multinomial logit_ (or, _generalized logit_) model.
...
**Anova**: Extends `Anova()` from the `car` package [@R-car],
It gives Type II tests for each term in the model. Note that the `LR` $\chi^2$ and `df` for the `Combined Responses` is the sum of their values for the separate dichotomies.
```{r Anova}
Anova(wlf.nested)
```
