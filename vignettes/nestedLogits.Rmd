...
**Coefficients**: The coefficients, $\mathbf{\beta_{i1}}$ give the changes in the
...
```{r update}
wlf.nested1 <- update(wlf.nested, formula = . ~ . + region)

anova(wlf.nested, wlf.nested1)
```

In a similar way, we could fit and test a larger collection of models.
...
The `directlabels` package can take a `ggplot` object, remove the legend and
