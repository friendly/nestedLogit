...
to.cite <- c("nnet", "car", "broom", "ggplot2")
```
...
* Vote in Quebec in a Canadian national election, (1) Liberal Party, (2) Conservative Party, (3) New Democratic Party, (4) Bloc Quebecois.
...
diagnosis can be interpreted in terms of the probabilities of being classified
into each of the response categories. 
<!-- Because of independence, analysis-of-deviance and coefficient tests for the  -->
...
For a principal example, we consider the data set `Womenlf` from the **carData package** [@R-car].
...
In equation form, the two log-odds models are:
...
In the `Womenlf` example, coefficients $\widehat{\beta}_{j1}$ gives the estimated change in the
...
Having young children also decreases the odds of both, by about 79% and 93% respectively.
...
These functions make it easy to construct custom tables. For example, to extract the likelihood-ratio difference-in-deviance ($G^2$)  tests and compute ($G^2 / df$):
```{r}
