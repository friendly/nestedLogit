...
extract_eq(wlf.nested,
           ...
           var_colors   = c(hincome = "red", children="darkgreen"))$work
```
...
displayed equation (the list element is still accessed as `eqns_gators$fish_inv`).
...
```{r gators-eqns-coef}
extract_eq(gators.nested, use_coefs = TRUE)$other
extract_eq(gators.nested, use_coefs = TRUE)$fish_inv
```
