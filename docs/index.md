...
model is the **proportional odds** model (e.g., fit by
[`MASS::polr()`](https://rdrr.io/pkg/MASS/man/polr.html)). This is a
simpler model, but achieves that simplicity by making the additional
assumption that the coefficients for the predictors are the same for all
categories.
...
to specify a collection of $`m-1`$ of these that can be used
...
- [`logits()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md):
  creates the set of dichotomies, typically using
  [`dichotomy()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md)
  for each.
- [`continuationLogits()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md):
  provides a convenient way to generate all dichotomies for an ordered
  response.
...
to facilitate working with `dichotomies` objects in other
representations. The `ABCD` example above corresponds to the matrix
below, whose rows represent the dichotomies and columns are the response
levels:
...
The result of
[`nestedLogit()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md)
is an object of class `"nestedLogit"`. It contains the set of
$`(m-1)`$[`glm()`](https://rdrr.io/r/stats/glm.html) models fit to the
dichotomies.
...
- `glance()` and `tidy()` are extensions of
...
dichotomies, using helper functions
[`dichotomy()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md)
and
[`logits()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md),
...
[`nestedLogit()`](https://friendly.github.io/nestedLogit/reference/nestedLogit.md)
effectively fits each of these dichotomies as logistic regression models
...
The structure can be shown nicely using
[`lobstr::tree()`](https://lobstr.r-lib.org/reference/tree.html):
...
extracted via
[`models()`](https://friendly.github.io/nestedLogit/reference/models.md).
These are the binomial [`glm()`](https://rdrr.io/r/stats/glm.html)
models.
...
`"nestedLogit"` objects. It can be called several times to give
multi-panel plots. By default, a 95% pointwise confidence envelope is
added to the plot. Here, they are plotted with `conf.level = 0.68` to
give $`\pm 1`$ std. error bounds.
...
![](reference/figures/README-wlf-plot-1.png)
...
- A collection of other examples of datasets for which nested logit
  models are useful, described in
  [`vignette("other-examples")`](https://friendly.github.io/nestedLogit/articles/other-examples.md)
  and [pkgdown
